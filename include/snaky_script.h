...
/**
  Define SNAKY_CUSTOM_DEFAULT_BUF_SIZE to change
  the default buffer size of SnakyScript.
*/
#ifdef SNAKY_CUSTOM_DEFAULT_BUF_SIZE
	#define SNAKY_BUF_SIZE SNAKY_CUSTOM_DEFAULT_BUF_SIZE
#else
	/**
	  The default buffer size of SnakyScript.
	*/
...
/**
  ...
  @param start_pos A pointer to a const char*. If a valid pointer
  is given, it will be equal to the position in the original
  string where the target argument's value was found. Example,
  in this argument string '<arg=ARG>' the start_pos pointer would
  point to the 'A' character after the '=.'

  @return 1 if the argument was successfully parsed, 0 if the
  function fails in any way.
*/
SNAKY_API int snaky_parse_target_arg(const char *str, char *buffer, size_t buffer_size, const char *arg_name, const char **start_pos);
/**
  ...
  To automatically walk a string and its arguments, pass a pointer into 'start_pos'
  and then on the next call use 'start_pos' as the argument string.

  @see snaky_parse_target_arg(const char*, char*, size_t, const char*, const char**)
*/
SNAKY_API int snaky_parse_arg(const char *str, char *name_buffer, size_t name_buffer_size, char *value_buffer, size_t value_buffer_size, const char **start_pos);
...
/**
  ...
  @param str The string holding the boolean argument value.
...
// TODO Make other quick parse_type(...) functions? like parse_int(...), parse_float(...), etc.
